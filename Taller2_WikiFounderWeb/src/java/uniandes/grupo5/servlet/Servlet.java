    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try{
            ...
            if (validar.contains("NO")){
                ...
                String[] textoArray = original.split("[||||]");                
                ArrayList<String> nodosArray = new ArrayList<String>();
                ArrayList<String> linksArray = new ArrayList<String>();
                                
                for (String renglon : textoArray){
                    if (renglon.contains("\"id\"")){
                        if (renglon.contains("\"id\"") && renglon.contains("\"source\"")){                            
                        }else{
                            nodosArray.add(renglon.toString());
                        }
                    }else if (renglon.contains("\"source\"")){
                        if (renglon.contains("\"id\"") && renglon.contains("\"source\"")){                                                        
                        }else{
                            linksArray.add(renglon.toString());
                        }                        
                    }
                };
                ...
                for (int j = 0 ; j<linksArray.size() ; j ++){
                    part = part + linksArray.get(j);
                }
                
                part = part.replaceAll("<br>", " ").replaceAll("<br />", " ").replace("1"+"{", "{").replaceAll("\\n", "_").replace("____", " ");
